#!/usr/bin/python
#coding:utf-8
import os, sys, re, filecmp, shutil
...
def compareme(dir1, dir2):  #递归获取更新项函数
    ...
    if len(dircomp.common_dirs) > 0: #判断是否存在相同的子目录，以便递归
        for item in dircomp.commom_dirs: #递归子目录
...
def main():
    if len(sys.argv) > 2: #要求输入源目录与备份目录
        ...
        print("Usage:", sys.argv[0], "datadir backipdir")
        sys.exit()
    ...
    for item in source_files:  #遍历返回的差异文件或目录清单
        ...
        if os.path.isdir(item):  #如果差异路径为目录且不存在，则在备份目录中创建
            if not os.path.exists(destination_dir):
                ...
                creatdir_bool = True  #再次调用compareme函数标记
                
    if creatdir_bool: #重新调用compareme函数，重新遍历新创建目录的内容
        destination_files = []
        source_files = []
    ...
    print("update item:")
    print(source_files)#输出更新项对应列表清单
    copy_pair = zip(source_files, destination_files)#将源目录与备份目录文件清单拆分成元组
    for item in copy_pair:
        if os.path.isfile(item[0]):#判断是否未文件，是则进行复制操作
